import { differenceInCalendarDays } from 'https://esm.sh/date-fns'
import { DBField } from './types.ts'

// This will take a list of fields and use the magic to predict
// the next paint date

export const getPredictedDaysUntilPaint = (
  field?: Pick<
    DBField,
    'max_dry_days' | 'last_painted' | 'rainfall_days' | 'rainfall_factor'
  >,
) => {
  if (!field?.last_painted) return

  return (
    field.max_dry_days -
    differenceInCalendarDays(new Date(), new Date(field.last_painted)) -
    field.rainfall_days * field.rainfall_factor
  )
}
